...
export class TechQuizPage implements OnInit {
  quizList: iQuiz[] = [];
  ...
  getCompletedLevels(): number[] {
    const storedLevels = localStorage.getItem('completedTechLevels');
  ...
  completeCurrentLevel(): void {
    ...
    if (!completedLevels.includes(this.currentLevel)) {
      ...
      localStorage.setItem('completedTechLevels', JSON.stringify(completedLevels));
  ...
  incrementCurrentLevel(): void {
    this.currentLevel++;
    localStorage.setItem('currentTechLevel', this.currentLevel.toString());
  }
